...
export function getDeltaToken(identityEmail: string, folder: string): string | null {
  ...
  ).get(identityEmail.toLowerCase(), folder) as { delta_link: string; updated_at: string } | undefined;
  if (!row) return null;
  const age = Date.now() - new Date(row.updated_at).getTime();
  if (!isNaN(age) && age > ROTATE_AFTER_MS) {
...
export function clearDeltaToken(identityEmail: string, folder: string): void {
  getDb().prepare(
    'DELETE FROM delta_tokens WHERE identity_email = ? AND folder = ?'
