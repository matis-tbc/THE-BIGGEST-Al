...
export function detectBounce(args: {
  ...
  const senderLooksLikeBounce = BOUNCE_SENDER_HINTS.some((h) => fromLc.includes(h)) ||
  ...
  const isAutoReplied = autoSubmitted.some((v) => v.includes('auto-replied') || v.includes('auto-generated'));

  const xFailed = headers['x-failed-recipients'] || [];
  const failedFromHeader = xFailed.flatMap((v) => v.split(/[,\s;]+/)).map((v) => v.trim()).filter(Boolean);

  const finalRecipMatches = args.body ? [...(args.body.matchAll(/final-recipient:\s*[^;]+;\s*([^\s<>]+@[^\s<>]+)/gi))] : [];
  const failedFromBody = finalRecipMatches.map((m) => m[1]);

  const failedRecipients = Array.from(new Set([...failedFromHeader, ...failedFromBody]
  ...
  const isBounce = (senderLooksLikeBounce || hasDsnContentType || failedRecipients.length > 0) &&
