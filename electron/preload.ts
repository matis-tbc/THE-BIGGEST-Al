...
declare global {
  interface Window {
    electronAPI?: {
      ...
      }) => Promise<{ submitted: number; failed: number; results: Array<{ recipientId: string; ok: boolean; messageId?: string; conversationId?: string; error?: string }> }>;
      onDispatchProgress: (runId: string, callback: (event: { index: number; total: number; result: { recipientId: string; ok: boolean; messageId?: string; error?: string } }) => void) => () => void;
      sendDrafts: (payload: { runId: string; messageIds: string[]; staggerSeconds: number; scheduledForIso?: string }) => Promise<{ sent: number; failed: number; results: Array<{ messageId: string; ok: boolean; error?: string }>; error?: string }>;
      ...
      onSendDraftsProgress: (runId: string, callback: (event: { index: number; total: number; result: { messageId: string; ok: boolean; error?: string } }) => void) => () => void;
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  dbMarkAllRepliesSeen: (identityEmail?: string) => ipcRenderer.invoke('db:mark-all-replies-seen', identityEmail),
  dbGetDeltaToken: (identityEmail: string, folder: string) => ipcRenderer.invoke('db:get-delta-token', identityEmail, folder),
  dbSetDeltaToken: (identityEmail: string, folder: string, deltaLink: string) => ipcRenderer.invoke('db:set-delta-token', identityEmail, folder, deltaLink),
  dbClearDeltaToken: (identityEmail: string, folder: string) => ipcRenderer.invoke('db:clear-delta-token', identityEmail, folder),
