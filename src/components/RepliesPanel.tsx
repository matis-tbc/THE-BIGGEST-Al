...
import { replyPoller, type PollerHealth } from "../services/replyPoller";
import {
  type TrackedReply,
  getReplies,
  markAllRepliesSeen,
  markReplySeen,
} from "../services/replyTracker";
...
const ClassificationBadge: React.FC<{ category: string }> = ({ category }) => {
  ...
  return (
    <span
      className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[9px] uppercase tracking-wide ${cls}`}
    >
      {label}
    </span>
  );
...
function healthTone(h: PollerHealth): { color: string; label: string } {
  ...
  if (ageMin < 2)
    return {
      ...
      label: `Polled ${ageMin < 1 ? "just now" : `${Math.round(ageMin)}m ago`}`,
    };
    ...
    <>
      ...
      >
        ...
        <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
          <path d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm2 0v.01L10 11l6-5.99V5H4zm12 2.408l-5.4 5.391a1 1 0 01-1.2 0L4 7.408V15h12V7.408z" />
        </svg>
        Replies
        {unread > 0 && (
            ...
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  Inbox replies matched to campaign sends
                </p>
              </div>
            ...
            {notifPerm === "default" && (
              <div className="border-b border-slate-800 px-4 py-2 bg-sky-500/5 flex items-center justify-between">
                <span className="text-[11px] text-slate-300">
                  Enable desktop notifications for new replies?
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={requestNotifications}
                    className="text-[11px] text-sky-300 hover:text-sky-200 px-2 py-0.5 rounded border border-sky-500/40"
                  >
                    Enable
                  </button>
                  <button
                    onClick={() => setNotifPerm("denied")}
                    className="text-[11px] text-slate-500 hover:text-slate-300 px-2 py-0.5"
                  >
                    Not now
                  </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {replies.length === 0 ? (
                <div className="p-6 text-sm text-slate-500">
                  No replies yet. The poller checks every 60s. Replies are matched by Outlook
                  conversation thread, so they'll appear here whether they reply directly to the
                  campaign email or to a CC.
                </div>
              ) : (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            {r.classification && (
                              <ClassificationBadge category={r.classification} />
                            )}
                          </div>
