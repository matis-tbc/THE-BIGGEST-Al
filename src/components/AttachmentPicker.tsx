...
export const AttachmentPicker: React.FC<AttachmentPickerProps> = ({
      ...
      <div className="space-y-6">
        <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-4">
          <p className="text-sm text-rose-300">
            File uploads are not supported in this environment. Please run Email
            Drafter inside the Electron desktop app to attach files.
          </p>
        </div>
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.info(
      "[AttachmentPicker] Selected file:",
      file.name,
      file.size,
      "bytes",
    );
    ...
    if (file.size > maxSize) {
      setError(
        `File size (${formatFileSize(file.size)}) exceeds the 150MB limit.`,
      );
      return;
    }
  ...
  const handleContinue = () => {
    if (selectedFile) {
      console.info(
        "[AttachmentPicker] Continuing with attachment:",
        selectedFile.name,
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">
          Select a file to attach to all draft emails. This file will be
          uploaded separately for each email.
        </p>
      </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-yellow-600/80">
                <p>
                  This file is larger than 3MB and will use the Large File
                  Attachment API. Upload may take longer for each email.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-cyan-500/80">
                <p>
                  This {formatFileSize(selectedFile.size)} file will be uploaded
                  separately for each email. Processing time will depend on your
                  internet connection and the number of contacts.
                </p>
              </div>
            </div>
          </div>
