...
export const CampaignDetail: React.FC<CampaignDetailProps> = ({
  ...
  const deleteSelectedCompanies = () => {
    ...
    const updated = {
      ...campaign,
      companies: campaign.companies.filter((c) => !selectedCompanies.has(c.id)),
    };
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode; count?: number }[] = [
    {
      ...
      count: campaign.companies.length,
    },
    {
      key: "contacts",
      label: "Contacts",
      icon: <Users className="h-4 w-4" />,
      count: campaign.contacts.length,
    },
    {
      key: "emails",
      label: "Email Finder",
  ...
  const statusBadge = () => {
    ...
    return (
      <span
        className={`text-xs font-medium px-2 py-0.5 rounded-full border ${styles[campaign.status]}`}
      >
        {campaign.status}
      </span>
    );
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            {editingDesc ? (
              <div className="flex items-center gap-2 mt-1">
                ...
                />
                <button onClick={saveDesc} className="text-emerald-400">
                  <Check className="h-4 w-4" />
                </button>
                <button
                  onClick={() => {
                    setEditingDesc(false);
                    setDescValue(campaign.description);
                  }}
          ...
          <div className="flex gap-2 ml-4 shrink-0">
            ...
            <button
              ...
              className="btn-secondary text-xs text-rose-400 border-rose-500/30 hover:bg-rose-500/10 flex items-center gap-1"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                {selectedCompanies.size > 0 && (
                  <button
                    ...
                    className="btn-secondary text-xs text-rose-400 flex items-center gap-1"
                  >
              ...
              {campaign.companies.length > 0 ? (
                      ...
                      <tr>
                        <th className="px-3 py-3 w-10">
                          <input
                            type="checkbox"
                            checked={
                              selectedCompanies.size === campaign.companies.length &&
                              campaign.companies.length > 0
                            }
                            onChange={() => {
                      ...
                      {campaign.companies.map((company) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <button
                              onClick={() => copyRow(company)}
                              className="text-slate-500 hover:text-slate-300 transition-colors"
                            >
                              {copiedRow === company.id ? (
                                <Check className="h-4 w-4 text-emerald-400" />
                              ) : (
                                <Copy className="h-4 w-4" />
                              )}
                ...
                <div className="rounded-xl border border-dashed border-slate-700 p-8 text-center bg-slate-800/50">
                  ...
                  <p className="text-sm text-slate-400 mb-3">
                    No companies yet. Use AI to discover relevant companies.
                  </p>
                  <button onClick={onOpenLeadGen} className="btn-secondary text-sm">
                    Generate Companies
                  </button>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              {campaign.contacts.length > 0 ? (
                ...
                <div className="rounded-xl border border-dashed border-slate-700 p-8 text-center bg-slate-800/50">
                  ...
                  <p className="text-sm text-slate-400 mb-3">
                    No contacts imported yet. Use Apollo/ContactOut to find contacts, then paste
                    them here.
                  </p>
                  <button onClick={onOpenContacts} className="btn-secondary text-sm">
                    Import Contacts
                  </button>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              {campaign.templateId && templateName ? (
                ...
                <div className="rounded-xl border border-dashed border-slate-700 p-8 text-center bg-slate-800/50">
                  ...
                  <p className="text-sm text-slate-400 mb-3">
                    No template selected. Choose one to use for this campaign's drafts.
                  </p>
                  <button onClick={onOpenTemplate} className="btn-secondary text-sm">
                    Select Template
                  </button>
                </div>
              )}
              ...
              {campaign.runs.length > 0 ? (
                    ...
                    <div key={run.id} className="card bg-slate-800 border-slate-700">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          {run.failCount > 0 && (
                            <span className="text-rose-400">{run.failCount} failed</span>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="rounded-xl border border-dashed border-slate-700 p-8 text-center bg-slate-800/50">
                  ...
                  <p className="text-sm text-slate-400">
                    No runs yet. Import contacts and select a template to run your first campaign.
                  </p>
                </div>
              )}
            </div>
